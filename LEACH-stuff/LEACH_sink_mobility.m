...
% Cluster Heads Election %
    ...
    DCIR = DCIR(1:CLheads);
    % Finding the minimum distance from mobile sink path
...
% Energy Dissipation for normal nodes %
    ...
    for i=1:n
       if (SN(i).cond==1) && (SN(i).role==0) && (CLheads>0)
        ...
        % Graph for routing of CHs


for i = 1:CLheads
    CL(i).dist=sqrt((CL(least_CH_id).x-CL(i).x)^2 + (CL(least_CH_id).y-CL(j).y)^2);
end
% G = graph();
% for i = 1:CLheads
%     if((findnode(G,CL(i).id)==0))
%     G=addnode(G,CL(i).id);
%     end
%      for j = i:CLheads
%           if((SN(CL(i).id).cond==1)&&(SN(CL(i).id).role==1)&&(i~=j))
%             if ((CL(i).dist<=threshold) && (findnode(G,CL(least_CH_id).id)~=0) && (findedge(G,CL(i).id,CL(least_CH_id).id)==0))
%                 G = addedge(G, CL(i).id, CL(least_CH_id).id, CL(i).dist);
%                 
% %              distance_1(i,j) = distance;
%             elseif(findnode(G,CL(least_CH_id).id)==0)
%                 G = addedge(G, i, CL(least_CH_id).id, CL(i).dist);
%             end
%             distance=sqrt((CL(i).x-CL(j).x)^2 + (CL(i).y-CL(j).y)^2);    
%             if(distance<threshold)
%                 G = addedge(G, CL(i).id, CL(j).id, distance);                     
%             end
%           end
%      end
% end
% %     [dist_closest_CH,closest_id] = min(distance_1(i,:));
% %     if(distance>threshold&&<CL(i).dcir)
% %         G = addedge(G, CL(i).id, CL(j).id, distance); 
%       
% 
% 
% for i = 1:CLheads
%     CL(i).route = shortestpath(G, CL(i).id,CL(least_CH_id).id);
% end  
...
% Energy Dissipation for cluster head nodes %
   ...
   for i=1:CLheads
     if (SN(CL(i).id).cond==1)  && (SN(CL(i).id).role==1 ) && (CL(i).id~=least_CH_id)
         if (SN(CL(i).id).E)>0
            ETx= (Eelec+EDA)*k + Eamp * k * CL(i).dist^2;
            SN(CL(i).id).E=SN(CL(i).id).E- ETx;
            energy=energy+ETx;
         end
         if (CL(i).id==least_CH_id)
             ETx= (Eelec+EDA)*k + Eamp * k * CL(i).dcir^2;
         end
         if  SN(CL(i).id).E<=0     % if cluster heads energy depletes with transmission
         dead_nodes=dead_nodes +1;
         operating_nodes= operating_nodes - 1;
         SN(CL(i).id).cond=0;
         SN(CL(i).id).rop=rnd;
...
%     function[distance] = dist_between(CL(i),CL(j))
%         distance=sqrt((CL(i).x-CL(j).x)^2 + (CL(i).y-CL(j).y)^2);
%     end
...
% function[coord] = circular_traj(radius)
% % % % % % 0.5,0.5 is origin
% crc = @(r,p,a) [(r)*cosd(a + p)+50;  (r)*sind(a + p)+50]; 
% t = linspace(0, 360, 120);                              % Time
% for i=1:length(t)
% coord(i).x = radius*cosd(t(i))+50;
% coord(i).y = radius*sind(t(i))+50;
% end
% init_pos = 0;                                          % Initial Position (°)
% locus = crc(radius, init_pos, t);
% figure(1)
% for k1 = 2:length(t)
%     plot(locus(1,k1),  locus(2,k1), 'bp')
%     viscircles([50,50],radius);
%     axis([-10  10    -10  10])
%     axis square
%     drawnow 
