...
Efs=10*10^(-12); % units in Joules/bit/m^2 (amount of energy spent by the amplifier to transmit the bits)
Emp=0.0013*10^(-12);
do=Efs/Emp
        ...
        for i=1:n
            ...
            if (SN(i).E>0) && (SN(i).rleft==0)
                generate=rand;	
                    ...
                    SN(i).dts=(sinkx-SN(i).x)^2 + (sinky-SN(i).y)^2; % calculates the distance between the sink and the cluster hea
...
% Grouping the Nodes into Clusters & caclulating the distance between node and cluster head %
       ...
       for i=1:n
        if  (SN(i).role==0) && (SN(i).E>0) && (CLheads>0) % if node is normal
            for m=1:CLheads
            d(m)=(CL(m).x-SN(i).x)^2 + (CL(m).y-SN(i).y)^2;
...
% Energy Dissipation for normal nodes %
    ...
    for i=1:n
       if (SN(i).cond==1) && (SN(i).role==0) && (CLheads>0)
       	if SN(i).E>0 && SN(i).dtch<do
            ETx= Eelec*k + Efs * k * SN(i).dtch;
            SN(i).E=SN(i).E - ETx;
            energy=energy+ETx;
        elseif SN(i).E>0 && SN(i).dtch>=do
...
% Energy Dissipation for cluster head nodes %
   ...
   for i=1:n
     if (SN(i).cond==1)  && (SN(i).role==1)
         if SN(i).E>0 && SN(i).dts<do
            ETx= (Eelec)*k + Efs * k * SN(i).dts;
            SN(i).E=SN(i).E - ETx;
            energy=energy+ETx;
         elseif SN(i).E>0 && SN(i).dts>=do
             ETx= (Eelec)*k + Emp * k * SN(i).dts^2;
            SN(i).E=SN(i).E - ETx;
            energy=energy+ETx;
         end
